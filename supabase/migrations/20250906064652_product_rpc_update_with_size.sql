...
-- 2) Recreate with per-slot filters + slot sorting + label in payload
create or replace function public.rpc_product_skus(
  ...
  _size_pos1       text  default null,    -- slot 1 text filter (nullable)
  _size_pos2       text  default null,    -- slot 2 text filter (nullable)
  _size_pos3       text  default null     -- slot 3 text filter (nullable)
)
returns table (
  sku_uuid uuid,
  product_uuid uuid,
...
begin
  -- Normalize free-text search into a wildcard
  ...
  order_by := case _sort_id
    when 'sku_code'            then format('%s %s NULLS LAST', 'b.sku_code',             case when _sort_desc then 'DESC' else 'ASC' end)
    when 'default_tax_code'    then format('%s %s NULLS LAST', 'b.default_tax_code',     case when _sort_desc then 'DESC' else 'ASC' end)
    when 'product_name'        then format('%s %s NULLS LAST', 'b._product_name',        case when _sort_desc then 'DESC' else 'ASC' end)
    when 'category_code'       then format('%s %s NULLS LAST', 'b._category_code',       case when _sort_desc then 'DESC' else 'ASC' end)
    when 'product_description' then format('%s %s NULLS LAST', 'b._product_description', case when _sort_desc then 'DESC' else 'ASC' end)
    when 'primary_barcode'     then format('%s %s NULLS LAST', 'b.primary_barcode',      case when _sort_desc then 'DESC' else 'ASC' end)
    when 'sku_short_code'      then format('%s %s NULLS LAST', 'b.first_short_code',     case when _sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos1'           then format('b.pos1n %s NULLS LAST, b.pos1t %s NULLS LAST', case when _sort_desc then 'DESC' else 'ASC' end, case when _sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos2'           then format('b.pos2n %s NULLS LAST, b.pos2t %s NULLS LAST', case when _sort_desc then 'DESC' else 'ASC' end, case when _sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos3'           then format('b.pos3n %s NULLS LAST, b.pos3t %s NULLS LAST', case when _sort_desc then 'DESC' else 'ASC' end, case when _sort_desc then 'DESC' else 'ASC' end)
    else                              format('%s %s NULLS LAST', 'b.updated_at',          case when _sort_desc then 'DESC' else 'ASC' end)
  end;
  ...
  $q$
    with base as (
      ...
      join public.product_item p
        on p.product_uuid = s.product_uuid
      where
        -- Free-text search across sku/product/barcodes/short-codes/sizes
        (
          $6::text is null
          ...
          or exists (
              select 1 from public.product_sku_short_code ss
          ...
          or exists (
              ...
              where z.sku_uuid = s.sku_uuid
                and (
                  lower(z.size_value) like $6 escape '\'
        ...
        -- If kind selected, require SKU to have at least one size row in allowed kind(s)
        and (
          ...
          or exists (
               ...
               where z0.sku_uuid = s.sku_uuid
                 and (
                      ($7::text[] is not null and z0.size_kind_code = any($7))
                   or ($8::text    is not null and z0.size_kind_code = $8)
                 )
             )
        )
        -- Per-slot filters (only meaningful when a kind is chosen)
        and (
          ($7::text[] is null and $8::text is null)
          or (
            ($9::text  is null or exists (
               ...
               where z1.sku_uuid = s.sku_uuid and z1.attr_pos = 1
                 and (
                      ($7::text[] is not null and z1.size_kind_code = any($7))
            ...
            and ($10::text is null or exists (
               ...
               where z2.sku_uuid = s.sku_uuid and z2.attr_pos = 2
                 and (
                      ($7::text[] is not null and z2.size_kind_code = any($7))
            ...
            and ($11::text is null or exists (
               ...
               where z3.sku_uuid = s.sku_uuid and z3.attr_pos = 3
                 and (
                      ($7::text[] is not null and z3.size_kind_code = any($7))
    ...
    with_codes as (
      select
        ...
        -- slot numeric/text for ORDER BY (not returned)
      ...
      left join lateral (
        ...
        where z.sku_uuid = b.sku_uuid
          and (
            ($7::text[] is null and $8::text is null)
            or ($7::text[] is not null and z.size_kind_code = any($7))
            or ($8::text    is not null and z.size_kind_code = $8)
          )
      ) sz on true
    )
    select
