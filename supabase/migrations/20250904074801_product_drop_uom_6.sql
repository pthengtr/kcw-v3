...
-- 1) Drop ALL existing overloads (any signature) of the function
do $$
declare
  r record;
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname = 'fn_product_create_full'
  loop
...
create function public.fn_product_create_full(
  _product_name        text,
  _category_code       text,
...
begin
  ...
  -- validate refs (category + optional tax); UOM checks removed
  ...
  -- sku_code (auto or validate) — unchanged
  ...
  -- upsert sku (UOM removed)
  insert into public.product_sku (
    sku_uuid, product_uuid, sku_code, default_tax_code,
    sku_short_code, is_active
  ) values (
    v_sku_uuid, v_product_uuid, v_sku_code, _default_tax_code,
...
-- 3) (Optional) grant execute to app role(s)
grant execute on function public.fn_product_create_full(text, text, text, text, text, text, boolean) to authenticated;
