...
begin
  ...
  if _default_tax_code is null or not exists (
      select 1 from public.product_tax_category t where t.tax_code = _default_tax_code
  ) then
  ...
  update public.product_item
   ...
   where product_uuid = _product_uuid;

  if not found then
    raise exception 'product not found: %', _product_uuid using errcode = 'P0001';
  end if;

  -- update sku (barcode will sync via your trigger on sku_code)
...
grant execute on function public.fn_product_update_full(
