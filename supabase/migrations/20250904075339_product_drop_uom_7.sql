...
-- 1) Drop ALL overloads of fn_product_update_full (any signatures)
do $$
declare
  r record;
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname = 'fn_product_update_full'
  loop
    execute format('drop function if exists %s;', r.sig);
  end loop;
end$$;

-- 2) Create UOM-free version
-- Required params first; optional params have defaults.
create function public.fn_product_update_full(
  _product_uuid        uuid,               -- required
  _sku_uuid            uuid,               -- required
  _product_name        text,               -- required
  _category_code       text,               -- required
  _product_description text default null,
  _default_tax_code    text default null,
  _sku_code            text default null,
  _sku_short_code      text default null,
...
begin
  ...
  -- validate refs (category + tax); UOM validation removed
...
-- grant execute on function public.fn_product_update_full(uuid, uuid, text, text, text, text, text, text, boolean) to authenticated;
