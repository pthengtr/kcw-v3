...
begin
  ...
  -- IMPORTANT: use alias **f** (not v) because final SELECT is "from filtered f"
  order_by := case sort_id
    when 'sku_code'            then format('%s %s NULLS LAST', 'f.sku_code',             case when sort_desc then 'DESC' else 'ASC' end)
    when 'default_tax_code'    then format('%s %s NULLS LAST', 'f.default_tax_code',     case when sort_desc then 'DESC' else 'ASC' end)
    when 'product_name'        then format('%s %s NULLS LAST', 'f._product_name',        case when sort_desc then 'DESC' else 'ASC' end)
    when 'category_code'       then format('%s %s NULLS LAST', 'f._category_code',       case when sort_desc then 'DESC' else 'ASC' end)
    when 'product_description' then format('%s %s NULLS LAST', 'f._product_description', case when sort_desc then 'DESC' else 'ASC' end)
    when 'primary_barcode'     then format('%s %s NULLS LAST', 'f.primary_barcode',      case when sort_desc then 'DESC' else 'ASC' end)
    when 'sku_short_code'      then format('%s %s NULLS LAST', 'f.first_short_code',     case when sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos1'           then format('f.pos1n %s NULLS LAST, f.pos1t %s NULLS LAST', case when sort_desc then 'DESC' else 'ASC' end, case when sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos2'           then format('f.pos2n %s NULLS LAST, f.pos2t %s NULLS LAST', case when sort_desc then 'DESC' else 'ASC' end, case when sort_desc then 'DESC' else 'ASC' end)
    when 'size_pos3'           then format('f.pos3n %s NULLS LAST, f.pos3t %s NULLS LAST', case when sort_desc then 'DESC' else 'ASC' end, case when sort_desc then 'DESC' else 'ASC' end)
    else                              format('%s %s NULLS LAST', 'f.updated_at',          case when sort_desc then 'DESC' else 'ASC' end)
  end;
    ...
    with filtered as (
      ...
      where
        (
          ...
          or exists ( select 1 from unnest(v.barcodes) b where lower(b) like $1 escape '\' )
          or exists ( select 1 from unnest(v.sku_short_codes) sc where lower(sc) like $1 escape '\' )
          or exists (
