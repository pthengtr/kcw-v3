...
-- Recreate with explicit size-kind filter (_size_kind_codes) and size_kind_label in output
create or replace function public.rpc_product_skus(
  ...
  _size_kind_codes text[] default null          -- explicit filter
)
returns table (
  sku_uuid uuid,
  product_uuid uuid,
...
begin
  ...
  -- Build search clause (always close ')' here)
  search_clause := '
    ($6::text is null
      ...
      or exists (
          ...
          where z.sku_uuid = s.sku_uuid
            and (
              ...
              lower(z.size_kind_code) like $6 escape ''\''      -- search by code
            )
      )';
  end if;
    ...
    sz_join := '
      left join lateral (
        ...
        where z.sku_uuid = b.sku_uuid
          and ($7::text[] is null or z.size_kind_code = any($7))   -- <<< use $7
      ) sz on true
    ';
    sz_fields := '
      sz.size_kind_code,
  ...
  'with base as (
   ...
   offset $4
   limit  $5'
  using _search, _category, _active, (_page_index * _page_size), _page_size, _needle_like, _size_kind_codes;  -- $1..$7
end;
$$;
